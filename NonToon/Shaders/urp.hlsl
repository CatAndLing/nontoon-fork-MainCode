...
half4 frag(v2f i, bool isFront : SV_IsFrontFace) : SV_Target
{
    ...
    // Custom Data
    // [NT-FIX 4] was `(SCCustomData)cd` - a self-referential cast of the variable being
    // declared (uninitialised garbage). Harmless only because the next line overwrites it.
    ...
    // [NT-FEAT 3] see birp.hlsl - view bias of the toon shade direction, now a property.
    sd.L = normalize(sd.L + vertex.Head * _ShadeDirectionBias + float3(0,1,0));
    // [NT-FEAT 1] lilToon-compatible lighting adjustment, kept in sync with birp.hlsl
    // (lilToon's LIL_CORRECT_LIGHTCOLOR_* : clamp(min,max) -> lerp(gray) -> lerp(1.0, AsUnlit)).
    half3 lightColor = env + lightSum.color;
    // [NT-FEAT 27] 亮度上下限 = **按亮度归一化**，不是逐通道 clamp。与 birp.hlsl 保持同步。
    ...
    // [NT-FIX 5] see birp.hlsl for the full rationale: HairSpecular and RimLight already
    // attenuate by sd.shadow, Specular did not. Revert by dropping `* sd.shadow`.
