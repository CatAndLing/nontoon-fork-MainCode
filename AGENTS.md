...
14. **用自己的命令行文本去匹配要操作的对象（进程／扫描结果）⇒ 自匹配。**
    同一天犯了**三次**：PowerShell 的 `Stop-Process` 匹配到自己的命令行而**自杀**（exit 4294967295）；
    孤儿清理脚本匹配到**运行自己命令的那个 bash** ⇒ 后半段整段没执行。
    ⇒ 替代做法：**锁文件／唯一 pid／显式排除自身**。**不是**「下次写仔细点」。
15. **`cmd | head` 读退出码 ⇒ 拿到的是 `head` 的退出码。** 同一天犯了**两次**：
    一次让安全扫描**恒报「检出密钥」**（**一个总是报警的扫描器等于没有扫描器**，
    与「不能失败的断言等于没有」是同一枚硬币）；一次把并发锁的「拒绝」读成了 exit 0。
    ⇒ 用 `X=$(cmd || true)` **接住**再判断，不要接管道。
16. **让 shell 元字符进入本该当「数据」的文本。** 把 `$(cmd)` 写进 `node -e "..."` 的字符串里，
    `\$` 没挡住命令替换 ⇒ bash 真执行了 `cmd` ⇒ **Windows 的 cmd.exe 启动横幅被嵌进 JS 源码**，
    脚本报语法错而整段静默失效（而我当时没读输出，以为改成了）。
    ⇒ 含元字符的文本一律**写成文件**，不要拼进命令行。
## 4. 已确立、不要重新讨论的结论
